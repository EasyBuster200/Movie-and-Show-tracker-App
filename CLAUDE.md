...
**Bookmarks are not a separate concept.** They're just the one `lists` row per user with `is_default = 1`, auto-created at signup. `bookmarks.html` is the UI for that one list; `lists.html` handles all lists including it. Don't add a parallel bookmarks table/API if extending this.
